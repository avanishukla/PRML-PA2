...
for i= 1: size_train_class1
        ...
        for z =  1: size(train1_cell{i,1},1)
            tmp = 0;
            for j = 1 : k1
                tmp = tmp + new_W(j)*exp(-0.5*(train1_cell{i,:}(z,:)-new_mean(j,:))*inv(new_cov{1,j})*(train1_cell{i,:}(z,:)-new_mean(j,:))')/(2*pi*sqrt(det(new_cov{1,j})));
            end
...
for i= 1: size_train_class2
        ...
        for z =  1: size(train2_cell{i,1},1)
            tmp = 0;
            for j = 1 : k2
                tmp = tmp + new_W(j)*exp(-0.5*(train2_cell{i,:}(z,:)-new_mean(j,:))*inv(new_cov{1,j})*(train2_cell{i,:}(z,:)-new_mean(j,:))')/(2*pi*sqrt(det(new_cov{1,j})));
            end
            p_given_theta = p_given_theta * log(tmp);
        end
        L_new = L_new + p_given_theta;
...
for i= 1: size_train_class3
        ...
        for z =  1: size(train3_cell{i,1},1)
            tmp = 0;
            for j = 1 : k3
                tmp = tmp + new_W(j)*exp(-0.5*(train3_cell{i,:}(z,:)-new_mean(j,:))*inv(new_cov{1,j})*(train3_cell{i,:}(z,:)-new_mean(j,:))')/(2*pi*sqrt(det(new_cov{1,j})));
            end
            p_given_theta = p_given_theta * log(tmp);
        end
        L_new = L_new + p_given_theta;
...
for i= 1: size_train_class4
        ...
        for z =  1: size(train4_cell{i,1},1)
            tmp = 0;
            for j = 1 : k4
                tmp = tmp + new_W(j)*exp(-0.5*(train4_cell{i,:}(z,:)-new_mean(j,:))*inv(new_cov{1,j})*(train4_cell{i,:}(z,:)-new_mean(j,:))')/(2*pi*sqrt(det(new_cov{1,j})));
            end
            p_given_theta = p_given_theta * log(tmp);
        end
        L_new = L_new + p_given_theta;
...
for i= 1: size_train_class5
        ...
        for z =  1: size(train5_cell{i,1},1)
            tmp = 0;
            for j = 1 : k5
                tmp = tmp + new_W(j)*exp(-0.5*(train5_cell{i,:}(z,:)-new_mean(j,:))*inv(new_cov{1,j})*(train5_cell{i,:}(z,:)-new_mean(j,:))')/(2*pi*sqrt(det(new_cov{1,j})));
            end
            p_given_theta = p_given_theta * log(tmp);
        end
        L_new = L_new + p_given_theta;
