...
cov_matrix3 = (cov_val_c3'*cov_val_c3)/250 ;
cov_matrix = (cov_matrix1+cov_matrix2+cov_matrix3 )/3 ;%%%%%%%%%%%%%%%%%
sigma_sq = 1000 ; %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for i=1:150 
    ...
    probab_val1_c1(i,1)= (exp(-0.5 * x'*inv(cov_matrix1)*x ) / sqrt(det(cov_matrix1)) );
    
    x(1,1) = val1(i,1)-mean2(1,1);
    x(2,1) = val1(i,2)-mean2(2,1) ;
    probab_val1_c1(i,2)= (exp(-0.5 * x'*inv(cov_matrix2)*x ) / sqrt(det(cov_matrix2)) );
    
    x(1,1) = val1(i,1)-mean3(1,1);
    x(2,1) = val1(i,2)-mean3(2,1) ;
    probab_val1_c1(i,3)= (exp(-0.5 * x'*inv(cov_matrix3)*x ) / sqrt(det(cov_matrix3)) );
end
...
for i = 1:150
 %valid 2
    ...
    probab_val1_c2(i,1)= (exp(-0.5 * x'*inv(cov_matrix1)*x ) / sqrt(det(cov_matrix1)) );
    
    x(1,1) = val2(i,1)-mean2(1,1);
    x(2,1) = val2(i,2)-mean2(2,1) ;
    probab_val1_c2(i,2)= (exp(-0.5 * x'*inv(cov_matrix2)*x ) / sqrt(det(cov_matrix2)) );
    
    x(1,1) = val2(i,1)-mean3(1,1);
    x(2,1) = val2(i,2)-mean3(2,1) ;
    probab_val1_c2(i,3)= (exp(-0.5 * x'*inv(cov_matrix3)*x ) / sqrt(det(cov_matrix3)) );
end
...
for i = 1:150
 %valid 2
    ...
    probab_val1_c3(i,1)= (exp(-0.5 * x'*inv(cov_matrix1)*x ) / sqrt(det(cov_matrix1)) );
    
    x(1,1) = val3(i,1)-mean2(1,1);
    x(2,1) = val3(i,2)-mean2(2,1) ;
    probab_val1_c3(i,2)= (exp(-0.5 * x'*inv(cov_matrix2)*x ) / sqrt(det(cov_matrix2)) );
    
    x(1,1) = val3(i,1)-mean3(1,1);
    x(2,1) = val3(i,2)-mean3(2,1) ;
    probab_val1_c3(i,3)= (exp(-0.5 * x'*inv(cov_matrix3)*x ) / sqrt(det(cov_matrix3)) );
end
disp(count);
count = 0;
...
for i = 1:100
    ...
    probab_test1_given_class(i,1)= (exp(-0.5 * x'*inv(cov_matrix1)*x ) / sqrt(det(cov_matrix1)) );
    
    x(1,1) = test_c1(i,1)-mean2(1,1);
    x(2,1) = test_c1(i,2)-mean2(2,1) ;
    probab_test1_given_class(i,2)= (exp(-0.5 * x'*inv(cov_matrix2)*x ) / sqrt(det(cov_matrix2)) );
    
    x(1,1) = test_c1(i,1)-mean3(1,1);
    x(2,1) = test_c1(i,2)-mean3(2,1) ;
    probab_test1_given_class(i,3)= (exp(-0.5 * x'*inv(cov_matrix3)*x ) / sqrt(det(cov_matrix3)) );
...
for i = 1:100
    ...
    probab_test2_given_class(i,1)= (exp(-0.5 * x'*inv(cov_matrix1)*x ) / sqrt(det(cov_matrix1)) );
    
    x(1,1) = test_c2(i,1)-mean2(1,1);
    x(2,1) = test_c2(i,2)-mean2(2,1) ;
    probab_test2_given_class(i,2)= (exp(-0.5 * x'*inv(cov_matrix2)*x ) / sqrt(det(cov_matrix2)) );
    
    x(1,1) = test_c2(i,1)-mean3(1,1);
    x(2,1) = test_c2(i,2)-mean3(2,1) ;
    probab_test2_given_class(i,3)= (exp(-0.5 * x'*inv(cov_matrix3)*x ) / sqrt(det(cov_matrix3)) );   
end
...
for i = 1:100
    ...
    probab_test3_given_class(i,1)= (exp(-0.5 * x'*inv(cov_matrix1)*x ) / sqrt(det(cov_matrix1)) );
    
    x(1,1) = test_c3(i,1)-mean2(1,1);
    x(2,1) = test_c3(i,2)-mean2(2,1) ;
    probab_test3_given_class(i,2)= (exp(-0.5 * x'*inv(cov_matrix2)*x ) / sqrt(det(cov_matrix2)) );
    
    x(1,1) = test_c3(i,1)-mean3(1,1);
    x(2,1) = test_c3(i,2)-mean3(2,1) ;
    probab_test3_given_class(i,3)= (exp(-0.5 * x'*inv(cov_matrix3)*x ) / sqrt(det(cov_matrix3)) );
...
for i = 1:250
    ...
    probab_train1_given_class(i,1)= (exp(-0.5 * x'*inv(cov_matrix1)*x ) / sqrt(det(cov_matrix1)) );
    
    x(1,1) = AM1(i,1)-mean2(1,1);
    x(2,1) = AM1(i,2)-mean2(2,1) ;
    probab_train1_given_class(i,2)= (exp(-0.5 * x'*inv(cov_matrix2)*x ) / sqrt(det(cov_matrix2)) );
    
    x(1,1) = AM1(i,1)-mean3(1,1);
    x(2,1) = AM1(i,2)-mean3(2,1) ;
    probab_train1_given_class(i,3)= (exp(-0.5 * x'*inv(cov_matrix3)*x ) / sqrt(det(cov_matrix3)) );   
end
...
for i = 1:250
    ...
    probab_train2_given_class(i,1)= (exp(-0.5 * x'*inv(cov_matrix1)*x ) / sqrt(det(cov_matrix1)) );
    
    x(1,1) = AM2(i,1)-mean2(1,1);
    x(2,1) = AM2(i,2)-mean2(2,1) ;
    probab_train2_given_class(i,2)= (exp(-0.5 * x'*inv(cov_matrix2)*x ) / sqrt(det(cov_matrix2)) );
    
    x(1,1) = AM2(i,1)-mean3(1,1);
    x(2,1) = AM2(i,2)-mean3(2,1) ;
    probab_train2_given_class(i,3)= (exp(-0.5 * x'*inv(cov_matrix3)*x ) / sqrt(det(cov_matrix3)) );   
end
...
for i = 1:250
    ...
    probab_train3_given_class(i,1)= (exp(-0.5 * x'*inv(cov_matrix1)*x ) / sqrt(det(cov_matrix1)) );
    
    x(1,1) = AM3(i,1)-mean2(1,1);
    x(2,1) = AM3(i,2)-mean2(2,1) ;
    probab_train3_given_class(i,2)= (exp(-0.5 * x'*inv(cov_matrix2)*x ) / sqrt(det(cov_matrix2)) );
    
    x(1,1) = AM3(i,1)-mean3(1,1);
    x(2,1) = AM3(i,2)-mean3(2,1) ;
    probab_train3_given_class(i,3)= (exp(-0.5 * x'*inv(cov_matrix3)*x ) / sqrt(det(cov_matrix3)) );
...
xaxis = -10:0.05:20;
yaxis = -20:0.05:20;
[xval, yval] = meshgrid(xaxis, yaxis);
plot_data = [xval(:) yval(:)];
...
for i= 1: total_plot_data
    ...
    probab_plot(i,3)= (exp(-0.5 * x1'*inv(cov_matrix3)*x1 ) / sqrt(det(cov_matrix3)) );
...
plot = reshape(predicted_plot, size(xval));
...
xlabel('x1')
ylabel('x2')
str  = { strcat('Decision Region Plot bayes diff linear')};
title(str,'FontSize',15);

t = text(-5,5,'CLASS 1','Color','r','FontSize',14);
